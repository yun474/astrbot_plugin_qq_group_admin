...
申请卡片展示申请 ID、昵称、时间、来源、风险提示和验证内容，隐藏成员 OpenID。
...
引用审批优先通过原通知消息 ID 定位；QQ 未提供或无法匹配该 ID 时，插件会读取引用正文里的「申请 ID」，仅匹配当前平台、当前群的待审记录，并检查操作者权限。也支持 `/同意`、`/拒绝 理由`。

> 无法定位申请、申请已处理或过期时，插件会明确提示，不再转入默认 LLM。旧通知若既无原消息 ID 也无申请 ID，请使用审批按钮。按钮发送失败时通知会降级为纯文本，仍保留申请 ID。
